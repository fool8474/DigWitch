...
void image::frameAlphaRender(HDC hdc, int destX, int destY, BYTE alpha)
{
	...
	{
		GdiAlphaBlend(hdc, destX, destY, _imageInfo->frameWidth, _imageInfo->frameHeight,
			_imageInfo->hMemDC, _imageInfo->frameWidth * _imageInfo->currentFrameX, _imageInfo->frameHeight * _imageInfo->currentFrameY, _imageInfo->frameWidth, _imageInfo->frameHeight, _blendFunc);
	}
}
